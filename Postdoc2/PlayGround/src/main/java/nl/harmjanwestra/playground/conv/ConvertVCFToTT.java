...
public class ConvertVCFToTT {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 2) {
            System.out.println("Usage: inputvcf outputfolder [maf:0.001] [missingness:0.05] [minAD:0]");
        } else {
            try {
                double maf = 0.001;
            ...
            if (!ln.startsWith("#")) {
                ...
                if (var.getAlleles().length > 2) {
                    ...
                    if (var.getMAF() < mafthreshold) {
                        ...
                        if (var.isIndel()) {
                            nrindels++;
                            if (var.getAlleles()[0].length() > var.getAlleles()[1].length()) {
                                allele1 = 73;
                                allele2 = 68;
                            } else {
                                allele1 = 73;
                        ...
                        if (missingness < missingnessthreshold) {
                            ...
                            if (var.isIndel()) {
                                snpMapFileWriter.append(var.getAlleles()[0] +" (" + allele1 + ")\t" + var.getAlleles()[1] + " (" + allele2 + ")");
                            } else {
                                snpMapFileWriter.append(allele1 + "\t" + allele2);
                            }
                            snpMapFileWriter.append('\n');
