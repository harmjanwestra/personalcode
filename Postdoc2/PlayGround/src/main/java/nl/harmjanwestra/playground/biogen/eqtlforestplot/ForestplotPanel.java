...
public class ForestplotPanel extends Panel {
    ...
    private ForestplotPanel.ORIENTATION orientation = ForestplotPanel.ORIENTATION.VERTICAL;
    ...
    public void setOrientation(ForestplotPanel.ORIENTATION orientation) {
    ...
    public void draw(DefaultGraphics defaultGraphics) {
        ...
        if (orientation == ForestplotPanel.ORIENTATION.VERTICAL) {
            g2d.setColor(greyScale[1]);
            g2d.setFont(deftheme.getSmallFont());
            int strw = getStringWidth(g2d, "Beta");
            g2d.drawString("Beta", x0 + (boxSize / 2) - strw, y0 - 20);
        }
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            if (orientation == ForestplotPanel.ORIENTATION.HORIZONTAL) {
    ...
    private void plotData(ArrayList<ForestPlotEQTL> events, int x1, int y1, String outcome, DefaultGraphics defaultGraphics) {
        ...
        if (orientation == ForestplotPanel.ORIENTATION.HORIZONTAL) {
            ...
            for (ForestPlotEQTL event : events) {
                ...
                g2d.fillOval(x1 + posX, y1 + getPos(event.beta, boxHeight, rangeBeta), dotsize, dotsize);
            ...
            for (ForestPlotEQTL event : events) {
                ...
                if (event.isMeta) {
                    int xp0 = x1 + getPos(event.beta, boxWidth, rangeBeta) - (dotsize / 2);
                    ...
                    g2d.fillOval(x1 + getPos(event.beta, boxWidth, rangeBeta) - (dotSizeForEvent / 2), y1 + posY, dotSizeForEvent, dotSizeForEvent);
                }


                // plot beta exposure confidence
                double conf1 = event.beta - 1.96 * event.se;
                double conf2 = event.beta + 1.96 * event.se;
                ...
                g2d.drawString(event.dataset, x3, y1 + posY);
                // event gene
                if (event.gene != null) {
    ...
    private void determineRanges(ArrayList<ForestPlotEQTL> events) {
        if (!forceRange) {
            ...
            for (ForestPlotEQTL e : events) {
                ...
                double valmin = e.beta - (1.96 * e.se);
                double valmax = e.beta + (1.96 * e.se);
